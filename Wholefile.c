...
void jadwalDokter(int preference) {
    for (dokter* currentDokter = head_dokter; currentDokter != NULL; currentDokter = currentDokter->next) {
        ...
        for (jadwal* currentHari = head_hari; currentHari != NULL; currentHari = currentHari->next) {
...
void printJadwal() {
    ...
    for (jadwal* day = head_hari; day != NULL; day = day->next) {
        printf("\n------------------------------------------- DAY %d -------------------------------------------\n", day->tanggal);
        char buffer[64];
        // Pagi
        printf("Pagi:\t");
